 ...
 * repository it went to, both generated by the publish.
 *
 * ⚠ **The coordinates are on the record and there is no `remote.json` here.** A Published Site may
 * point back at its repository; it may not carry a Remote relationship, which is installation-local
 * metadata keyed by Workspace identity. {@link legacyPublishedByEditor} is the shape of a site
 * published before that separation, and the fallback that reads it.
 */
...
/**
 * A site published before the record carried its repository: the coordinates are in `remote.json`
 * inside the published tree instead (ADR-0032).
 *
 * Every such site is in front of Readers now, and the links they carry have to keep working — so one
 * of the two return-link specs below reads this shape rather than the current one.
 */
function legacyPublishedByEditor(): SiteFiles {
	return {
...
test.describe('a Published Site a Reader arrives at', () => {
	...
	test('leads back to the editor that published it, from the Front Page and from a Project', async ({
		...
		// addresses**. Those sites are in front of Readers now, with the coordinates in
	 ...
	 *
	 * Every site published before the editor recorded its own address is in this state and is in front
	...
	test('says nothing about an editor when the site does not record one', async ({ page }) => {
		...
		await expect(page.getByTestId('site-problem')).toHaveCount(0);
		// Every such site is in this state, and the binding would be a whole extra round trip on the
...
test.describe('exploring a Project', () => {
	...
	test('draws the stack in the author’s order with zoom at the bottom-left, the Annotation Layer above the map Layer, and the Inspector docked over the pane’s top-right', async ({
		...
		// budget is spent (`scripts/check-seam-2-size.mjs`), and this test already has a published
...
test.describe('a Map Image read unwarped', () => {
	 ...
	 *
	 * The Project here is one the author published **with an address**, so
...
test.describe('a Published Site that is not entirely well', () => {
	...
	test('warns that a referenced Map Image leaves a Reader with no network seeing nothing', async ({
		...
		// Image's tiles are held is the author's publishing decision: a Reader cannot copy a pyramid,
