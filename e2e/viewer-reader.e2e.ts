...
test.describe('a Published Site a Reader arrives at', () => {
	...
	test('serves the hub and one Project over plain HTTP, at a domain root and in a subdirectory', async ({
		...
		for (const served of site.sites) {
			...
			// published into a folder, and of every site published before the record carried either —
	...
	/**
	 * A site published to a folder rather than to a Remote records no repository, so there is nothing
