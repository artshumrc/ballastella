...
// Every catalog entry reads a remote archive (ADR-0025), so a Reader test that wants a map to
...
/**
 * Seam 2 for the read-only experience a Reader gets from a Published Site, driven in a real browser
 * against a real static server.
 *
 ...
 * the same arrangement `editor-sync.e2e.ts` uses for `ballastellaServedTiles`. The shapes are read
 ...
 * there is. So each payload's `prose` is asserted **present** before anything about what is absent.
 */
const PAYLOADS = [
	{
...
/**
 * A site as a Sync to a Remote leaves it: the record naming the instance that wrote it and the
 * repository it went to, both generated by the site write.
 *
 * ⚠ **The record is the only place the repository is.** A Published Site may point back at its
 * repository; nothing in the tree describes a Remote *relationship*, which is installation-local
...
// ─────────────────────────────────────────────────────────────────────────────────────────
// THE LAYER STACK, WHICH IS THE EDITOR'S CARD
//
...
/**
 * Every control the shared card offers the editor and must never offer a Reader.
 *
 * ⚠ **These absences are not asserted alone**, which is the rule this file keeps: every id below
 ...
 * shared card's empty state was the editor's own guidance verbatim, so a Project on a site with no
 ...
 *
 * The last three are the sentences the map pane's shared components carry past: the editor's own
 * empty-stack line, which `MapCommentary` renders through a snippet rather than composing, and its
 * offline reassurance, which `MapNotice` takes as children. Both are one edit away from being written
 * into shared code — that edit has been made four times already — and neither was matched by anything
 * above: "this Workspace" is not "your Workspace", and an `sr-only`
 ...
 *
 * Named here because a site's cached tiles sit in a directory keyed on it, and because
 ...
 *
 * **A pin selects the Annotation's own row and no popup is drawn**, and the Annotation is
 ...
 *
 * **This is where a Reader actually reads an Annotation**: the Annotation Layer
...
/**
 * Everything an author can do to an Annotation, which must reach no Reader.
 *
...
const ANNOTATION_EDITING_CONTROLS = [
	...
	// **No tab strip at all, and not a lone Text tab**: the
...
// ─────────────────────────────────────────────────────────────────────────────────────────
// SANITISATION. The one place in this suite where a bug is a security vulnerability.
// ─────────────────────────────────────────────────────────────────────────────────────────

test.describe('untrusted text on a Published Site', () => {
	/**
	 * Markdown sanitisation is asserted **twice on purpose**: where it is written, in
	 * `packages/core/src/annotation/markdown.browser.test.ts`, and again here, because a Published
	 * Site runs on the author's own domain — `student.github.io`, or
	 * `maps.digitalhumanities.harvard.edu` — and the Project it renders may have arrived from a
	 * stranger by zip import or from a remote library.
	 *
	 * **Two surfaces, and they are safe for different reasons.** The distinction is what stops a future
	 * edit "simplifying" the wrong one:
	 *
	 ...
	 * popup retired from the Project screen in both apps, and the row stopped revealing
	 ...
	 * shipped bundle and a `{@html}` hydrating permanently blank would be caught. That surface was
	 * Reader-side popup behaviour, so it went with the app's last `{@html}`. Nothing was lost: the
	 * tests below load a Published Site in this build and select a real Annotation, so they already
	 * prove the shared path is live here — and each of them asserts the prose arrived *before*
	 * asserting what did not, which is the same blank-surface guard the prose block was carrying.
	 */
	...
	test('a Project’s name and a Layer’s name are text, never markup — a different mechanism', async ({
		...
		//
		// ⚠ **The name, not the card around it.** The card is the editor's own, and it
	...
	test('parses before sanitising, so the anchor the parser builds is seen by the sanitiser', async ({
		...
		// ─────────────────────────────────────────────────────────────────────────────────────
		// THE TWO-SIDED CLAIM, IN THE INSPECTOR
		//
...
test.describe('a Published Site a Reader arrives at', () => {
	...
	test('serves the hub and one Project over plain HTTP, at a domain root and in a subdirectory', async ({
		...
		site = await servedSite(oneProject());
		...
		for (const served of site.sites) {
			...
			// claim**. This fixture records no editor instance and no repository — the state of a site
			// written into a folder, and of every site written before the record carried either —
			...
			// **The other half of this pair is in `leads back to the editor that wrote it`**, which
	 ...
	 * `leads back to the editor that wrote it` asserts.
	 */
	test('renders the same blank Front Page for both kinds of empty site', async ({ page }) => {
		...
		// A site with nothing on it: no Project files at all, and a record listing none.
		await site.close();
		site = await servedSite({ 'ballastella-site.json': siteRecord([], writtenFrom()) });
		await page.goto(site.sites[0]!.url);

		await expect(page.getByRole('link', { name: RETURN_LINK })).toHaveCount(0);
		// An empty Front Page is not a fault: no alert, and no invitation to go looking for the files.
		await expect(page.getByTestId('site-problem')).toHaveCount(0);
	 ...
	 * are built from a record read relative to the document: its `editorUrl` and its `repository`. A
	...
	test('leads back to the editor that wrote it, from the Front Page and from a Project', async ({
		...
		site = await servedSite(writtenByEditor());
		...
		for (const served of site.sites) {
			...
			// student with no GitHub account is exactly the Reader who will not follow a link that
			// looks like it wants one. **Present here and absent in `serves the hub
			...
			// list of Projects, and the theme control where the editor keeps its own.
			...
			// The same bar, saying where the Reader is now.
			...
			// And the site's own name is the way home, in the same place on every page.
	...
	/**
	 * A record with no instance address renders no link and no error.
	 *
	 * Every site written before the editor recorded its own address is in this state and is in front
	 * of Readers now. A guess at a canonical deployment would be worse than the absence: it would offer a stranger's editor a
	...
	test('says nothing about an editor when the site does not record one', async ({ page }) => {
		...
		await expect(page.getByRole('link', { name: RETURN_LINK })).toHaveCount(0);
		await expect(page.getByTestId('site-problem')).toHaveCount(0);
		// ⚠ **And no second file is looked for.** The record is the only account a site has of its own
		// repository (ADR-0044), so a Front Page that came up empty-handed here goes nowhere else —
		// which would have been a whole extra round trip on every such site.
	...
	/**
	 * A site written into a folder rather than sent to a Remote records no repository, so there is nothing
	 * to name in a link — and a Reader meets a Front Page, not a 404 or an alert.
	 */
	test('says nothing about an editor when the site is bound to no repository', async ({ page }) => {
		site = await servedSite(oneProject({}, { editorUrl: EDITOR_INSTANCE }));
		const seen = watch(page);
	...
	test('reads everything through the HTTP store, and makes no request that could change a byte', async ({
		...
		// and changing a view control makes **no** write attempt and produces no error.
		//
		...
		// **And there is nothing here that could be an edit**. Asserted in the
...
test.describe('exploring a Project', () => {
	...
	test('draws the stack in the author’s order with zoom at the bottom-left, the Annotation Layer above the map Layer, and the Inspector docked over the pane’s top-right', async ({
		...
		site = await servedSite(oneProject({ annotations: numbered }));
		const seen = watch(page);

		await page.goto(site.sites[0]!.url + '?p=amsterdam-1625');
		await mapReady(page);

		// Zoom is at the bottom-left in the reader pane as in every other map pane, asserted against the
		// rendered control rather than the call that placed it: MapLibre creates all four corner
		// containers whatever is put in them, so the claim is which corner holds the buttons.
		...
		// ─────────────────────────────────────────────────────────────────────────────────────
		// AND THE READER SEES THE SAME NUMBERS THE SCHOLAR DOES
		//
		...
		// against a pin, a line and a shape; this is the same rule reaching the real Reader build over
		...
		// (`scripts/check-seam-2-size.mjs`), and because this test already has a Project on a site with
		...
		// ─────────────────────────────────────────────────────────────────────────────────────
		// AND THE LEADER JOINS THE OPEN ROW TO ITS PIN
		//
		// The line the editor draws, drawn by the same component in the real Reader build. Folded in
		// here for the reason the numbering above was: the Seam 2 budget is spent, and this test
		// already has a Project on a site with a drawn Annotation Layer on a real map.
		//
		// ⚠ **The canvas end is asserted against `map.project()` of the coordinate in the site's
		...
		//
		// A Published Site looks like the tool it was made in, so the Reader's panel is the author's
		// panel in the same corner of the same kind of pane. **A layout claim, so it is Seam 2 or
		// nothing** — there is no `offsetWidth` and no paint at Seam 1c. Folded in here rather than
		// given a test of its own for the reason the numbering and the leader above were: the Seam 2
		// budget is spent (`scripts/check-seam-2-size.mjs`), and this test already has a site's
		// Project with a drawn Annotation Layer on a real map and a row just selected.
		//
		...
		// **The panel is never under the zoom control** without a second assertion for it: the
		...
		// map draws that Annotation more strongly.
		await expect(leaderLayer(page)).toHaveAttribute('aria-hidden', 'true');

		// ── AND ON A NARROW SCREEN THERE IS NO LINE AT ALL ───────────────────────────────────
		//
	...
	test('hiding a Layer takes it off the map and off the count, and shows it again', async ({
		...
		// equivalent merges over a record it never prunes, so a hidden Layer went on being counted as
		// drawn — a defect recorded against the editor's Layer stack in
		// `apps/editor/src/lib/project/ProjectScreen.svelte`, and the reason this count is built from
		// the Layers that are currently shown and nothing else.
		site = await servedSite(oneProject());
		const seen = watch(page);
	...
	test('opacity reaches the warped Layer in place, without rebuilding the stack', async ({
		...
		// The slider is inside the Layer's own card, which a Reader opens — the same step the editor's
		// suite takes.
...
// ─────────────────────────────────────────────────────────────────────────────────────────
// A READER MEETS THE AUTHOR'S LABELS
//
// **Against the real Reader build, and that is the whole of why these two tests are here.** A Label
...
// below needs a line of Label-specific code in `apps/viewer`. What a Published Site *can* break is
...
// second application. "A Published Site changes nothing about how the work looks" is therefore a claim only
...
test.describe('a Published Site draws the author’s Labels', () => {
	...
	/** The Workspace this section shares: one Annotation Layer of Labels, and one ordinary Pin. */
	...
	test('draws every Label from the Label bucket, in the colour, at the size and at the place its file states', async ({
		page
	}) => {
		site = await servedSite(labelledProject());
		const served = site.sites[0]!;
		const seen = watch(page);
		...
		// ── DRAWN, AND OUT OF THE LABEL BUCKET ──────────────────────────────────────────────
		//
		...
		for (const size of ['small', 'medium', 'large'] as const) {
			...
			// drawing: this Project was parsed out of static HTTP by the Reader build, and these are
		...
		// ── AT THE AUTHOR'S SIZE, AND IN THE AUTHOR'S COLOURS ───────────────────────────────
		//
		// Identical words on all three, so the only thing that can separate these widths is
		// `marker-size` reaching `text-size` through the Reader build.
		const chips = {
	...
	test('opens a clicked Label’s Layer card with it selected, and reads its words and description with nothing to edit', async ({
		page
	}) => {
		site = await servedSite(labelledProject());
		const served = site.sites[0]!;
		...
		// which is what this asserts survived reaching the site.
		...
		// ── ITS WORDS IN THE IDENTITY HEADER, AND ITS DESCRIPTION IN THE FACE ───────────────
		//
		...
		// ── AND NOTHING AT ALL TO EDIT IT WITH ──────────────────────────────────────────────
		//
		...
		//
		// A Label needs no case of its own: this is the description face's own answer to an absent
		// `description`, reached by the path an undescribed Pin already takes.
		await page
...
test.describe('the Base Map a Reader sees', () => {
	...
	test('offers the catalog that travelled with the site, not this build’s (ADR-0020)', async ({
		...
		site = await servedSite(oneProject({}, { baseMap: travelled }));
		const seen = watch(page);
	...
	test('offers the entries that need a network without spelling the caveat into every label', async ({
		...
		// ADR-0020 requires that a Reader offline is *told* why the map is blank rather than left with
		// it — but not that every remote entry carry a suffix, which made the switcher
...
test.describe('a Map Image read unwarped', () => {
	 ...
	 *
	 * The Project here is one the author stamped **with an address**, so
	...
	for (const entry of ['by link', 'by loading the URL'] as const) {
		test(`opens over HTTP ${entry}, and the navigation throws nothing`, async ({ page }) => {
			...
			// the site is served first and the `info.json` rewritten afterwards, which is the order the site write
			...
			// and not in the editor: a pyramid on a site has a URL. Counted from `from`, so the warped map's own
	...
	test('says so when the image behind a Map Image is not on the site', async ({ page }) => {
		site = await servedSite(oneProject({ withoutPyramid: true }));
		const seen = watch(page);
		...
		// is where it is on every other screen of the site — this one included.
		const bar = page.getByTestId('navigation-bar');
...
test.describe('a Published Site that is not entirely well', () => {
	...
	/**
	 * The whole sentence a site with none of the Base Map's display assets shows a Reader.
	 *
	 ...
	 *
	 * It names the author's Labels because a Label's words are shaped from these very
	...
	test('names the host when a referenced Map Image’s record cannot be read', async ({ page }) => {
		// The degradation rule `unreadable` states in full (`apps/viewer/src/lib/project-documents.ts`):
		// "Referenced image whose host is unreachable → say so, naming the host; keep the rest of the
		// site working."
		//
		...
		// blank while the page reports it drawn. That is the recorded defect, and the reason this Layer
		// is `unreadable` instead.
		//
	...
	test('falls back with a quiet notice when the Base Map id is absent from the catalog', async ({
		...
		site = await servedSite(oneProject({ baseMap: 'a-base-map-from-another-deployment' }));
		const seen = watch(page);
	...
	test('draws the work over a network Base Map without shipping a tile archive', async ({
		...
		// **Empty rather than absent**, and that is the mechanism rather than a detail: this
		...
		// ── AND A LABEL DRAWS, BECAUSE THIS SITE CARRIES THE TYPEFACES ───────────────────────────
		//
	...
	test('says so when the site carries no copy of the Base Map’s labels and symbols', async ({
		...
		// Legacy Published Sites may not carry the Base Map's own glyphs and sprites. Every site this build writes
		// include them, but the Reader still has to explain this older state rather than silently
		// dropping every place name.
		//
		// Every catalog entry reads a remote archive, so the geography still draws — and what a site
		// without those files loses is **every place name on the map**. `ReaderMapPane` drops `glyphs`,
		// `sprite`, and the symbol layers rather than 404ing at files that are not there, and the
		// question this test exists to answer is whether the Reader is told. A geography-only map with
		// no account of itself is precisely the silent failure this test forbids.
		//
		// **And the author's own Labels go with the place names**, because a Label's words are shaped
		// from those same typefaces. So this Project holds one of every kind: the Label must not be
		...
		// ── THE LABEL IS NOT DRAWN, AND THE REST OF THE LAYER IS ────────────────────────────────
		//
	...
	// ═════════════════════════════════════════════════════════════════════════════════════════════
	// AN ARCHIVE THAT DOES NOT ANSWER, SAID OUT LOUD TO A READER
	//
	// The editor asserts this too, but the viewer is the side that needed it more: `ReaderMapPane`
	// says so itself — "a Published Site has no console anyone is watching". A Reader handed an empty
	// rectangle cannot tell an outage from a broken tool, and cannot rule out the third possibility
	// either, that the scholar's work failed to draw.
	//
	...
	/**
	 * The whole sentence, for the entry these sites were written with and the host this
	...
	test('says so when the Base Map’s archive answers nothing, and keeps drawing the work', async ({
		...
		site = await servedSite(oneProject({}, { baseMapAssetsBundled: true }));
		const served = site.sites[0]!;
		const seen = watch(page);
		...
		// (ADR-0016, and the amendment to it the shared component holds).
		await expect(notice).toHaveAttribute('role', 'alert');
		...
		// page's own count of itself, which is exactly the kind of evidence this suite refuses. Tiles
	...
	test('withdraws the outage notice when the Reader’s own connection goes', async ({
		...
		// ── No exception, and that is the patch's proof ─────────────────────────────────────────────
	...
	test('says both things when a site with none of those files also meets an outage', async ({
		...
		// prose that these two notices were mutually exclusive — that the bare style "declares
		...
		// absolute, so a site with none of its Base Map files draws the remote style with its
		...
		// there. The claim about the geography is gone rather than made conditional — see the next
		// test for the row a conditional one would still have got wrong.
		await page.unroute(/\.pmtiles$/);
		await refuseBaseMapArchive(page);

		site = await servedSite(oneProject({}, { baseMapAssetsBundled: false }), {
			withoutBaseMap: true
		});
		const seen = watch(page);
		...
		// Reader's to work around by switching Base Map, the missing labels are the author's to fix.
		...
		// silently lose one of the two announcements.
	...
	test('says the same thing about its missing labels with no connection at all', async ({
		...
		await expect(page.getByTestId('base-map-not-in-site')).toHaveText(NOT_IN_SITE_NOTICE);
	...
	test('takes the notice down when the archive starts answering again', async ({ page }) => {
		...
		// the silence the notice exists to end.
		//
	...
	test('refuses a Project from a newer version plainly, naming where to open it (ADR-0010)', async ({
		...
		site = await servedSite(oneProject({ projectOverrides: { formatVersion: 99 } }));
		const seen = watch(page);
	...
	test('tells a Reader the site has no list of Projects yet, rather than throwing', async ({
		...
		// is what the viewer's own files look like before a site record exists.
		site = await servedSite({});
		const seen = watch(page);
	...
	test('warns that a referenced Map Image leaves a Reader with no network seeing nothing', async ({
		...
		// The Reader's side of a referenced Map Image. What decides whether a Reader needs the network is
		// the **site's own files** — a `remote.json` with no `info.json` of ours beside it (ADR-0023) —
		// rather than a field of `project.json`, and the Reader is the person who meets the consequence.
		site = await servedSite(
			await oneProject({
				imageMode: 'referenced',
				remoteService: 'https://maps.library.example/iiif/x'
		...
		// ⚠ **And no per-Layer badge, which is a different thing from the paragraph above.** Where a Map
		// Image's tiles are held is the author's own decision: a Reader cannot copy a pyramid,
		// cannot repoint a service, and cannot make the badge say the other thing. The warning above
		// survives because it names what will not draw on a train, which is a consequence a Reader meets.
		//
	...
	// ═════════════════════════════════════════════════════════════════════════════════════════════
	// A MAP IMAGE WHOSE TILES STOP ARRIVING, SAID OUT LOUD
	//
	...
	test('tells a Reader when a Map Image’s tiles stop arriving, and keeps what arrived', async ({
		...
		// Visible text and not a tooltip (ADR-0016), and **the whole sentence**, built in
		// `support/tile-failure-notice.ts` rather than written out here: that is what makes "the editor
		// and the viewer say the same thing about the same failure" a contract instead of an intention,
		// and what makes a reword in core turn this red. One assertion, because any fragment of it would
		// be a substring of the string already pinned exactly and so could never fail on its own.
		//
		...
		// Driven as error → recovery, with the notice asserted visible above so this cannot pass by
		// never having raised it. The gesture is the one the sentence names.
		await redrawMapLayer(page);
		await expect(notice).toHaveCount(0, { timeout: 45_000 });
		await expect.poll(() => cachedTiles(page), { timeout: 60_000 }).toBeGreaterThan(0);

		// ── And nothing was thrown, on any of it ────────────────────────────────────────────────────
		// The whole point of the remedy. This is the assertion an escaping rejection fails, and it
		// covers a refusal driven deliberately rather than one that happened to race.
		expect(seen.failures).toEqual([]);
	});
	...
	test('takes the notice down by itself when the map’s own record answers again', async ({
		...
		// ═══════════════════════════════════════════════════════════════════════════════════════════
		// THE HALF THAT REALLY DOES HEAL WITH NO GESTURE AT ALL
		//
...
test.describe('a Reader on a phone', () => {
	// The one surface with a genuine mobile requirement: authoring is desktop-only and settled
	// (ADR-0014), but a published site is what a scholar shows colleagues and what most people will
	// ever see. A desktop-only run would pass while the phone experience stayed broken, so the viewport
	// here is real.
	...
	/**
	 * The bar folds, and the return links read well at 375 px.
	 *
	...
	test('tapping an Annotation reads it in a sheet at the bottom of the map, with the same subtraction and no leader', async ({
		...
		// **The phone is why the popup retired**. A 375 px screen has no room for a bubble
		...
		// **Nothing opened inside the row**, which is what
		...
		//
		// ⚠ **A sheet is the layout that reinstates the fault the dock decision was taken to remove.**
		// The sheet is `z-index: 7` and `layout.css` pins MapLibre's control corners to 6, so a
		// full-width band across the pane's bottom edge covers the zoom control the dock decision moved
		// to the bottom-left *precisely* so that it could never be under the Inspector. Measured on this
		...
		// what matters is whether the button can be pressed. The overlap is asserted beside it so that a
		// sheet clearing the centre and covering the corners cannot pass, and the button is then really
		// pressed and the zoom really read.
...
test.describe('a Reader using a keyboard', () => {
	...
	test('reaches and operates every control by tabbing, and hears what changed', async ({
		...
		// Reached by tabbing rather than by locating and clicking, because a control that a locator can
		// click is not necessarily one a keyboard can get to.
		site = await servedSite(oneProject());
		const seen = watch(page);
		...
		// is reached and operated with nothing to learn.
...
//
// ADR-0026 names the Published Site as **the half most likely to be forgotten**: nothing else would
// fail if the viewer went on opening on the deployment's default while the editor opened on the
// author's work. The Reader is the one person who could not tell.
//
 ...
 * assertable at all: the Reader's map handle is announced by `drawLayerStack`, so a Project with an
...
test.describe('a Published Site opens on the Project’s content', () => {
	...
	test('frames on the work, at both base paths, exactly as the editor frames it', async ({
		...
		// And says where it is looking, in an announced live region.
	...
	test('caps the zoom on a Project whose only content is one pin', async ({ page }) => {
		site = await servedSite(pinnedProject([BOSTON_PINS[1]!]));
	...
	test('frames on a sheet whose Alignment reads, even when its image record does not', async ({
		...
		// ways.
		//
		// Set up with a failure the viewer's degradation rules already cover: the `remote.json` that says
		// which server holds the tiles is answered 503. The Alignment beside it is untouched and puts
		// the sheet in Boston, four thousand miles from this deployment's default — so "framed on the
		...
		site = await servedSite(
			oneProject({
				...
				// the stack is built and the Reader's map handle is announced.
				annotations: []
			})
		);
		await page.route('**/images/aaa/remote.json', (route) =>
	...
	test('does not move the map when a Reader hides a Layer, and re-frames when asked', async ({
		...
		// Hidden, then shown again. The handle is announced again by `drawLayerStack`, so it is gone while the
