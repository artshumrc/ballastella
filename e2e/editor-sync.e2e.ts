...
/**
 * Seam 2 for syncing, and the assertion the whole of it rests on.
 *
 ...
 * make the byte-identity assertions below fail whenever a Sync swept litter an earlier write left,
...
const projectFiles = (
	...
	// alignment-write-is-the-fixture: the Workspace this spec syncs from, laid down before the app starts
 ...
 *
 * A credential is seeded unless `signedIn: false`, because syncing is not offered without one and
 ...
 * test here is about the files a Sync writes.
 */
...
/**
 * Sync, and wait for the announced result.
 *
 */
/**
 * The one fake GitHub a spec's syncs all reach.
 *
 * ⚠ **One per spec, and that is a correctness requirement now rather than tidiness.** Installing the
 * routes again builds a *new* repository with nothing in it, and Sync is baseline-aware: a Remote
 * that has lost every source path this machine's Baseline says it holds is an inbound deletion, so
 * the second Sync is correctly refused and told to get first. The reset was invisible while the
 ...
 * arrives holding a credential (see {@link openWorkspace}), so the Sync dialog asks GitHub what
...
async function prepareSync(page: Page, dialog: ReturnType<Page['getByRole']>) {
	await routeGitHubOnce(page);
...
async function sync(page: Page, existingDialog?: ReturnType<Page['getByRole']>) {
	const dialog = existingDialog ?? (await openSyncModalWithShareLinks(page));
	await prepareSync(page, dialog);
...
test.describe('syncing a Workspace', () => {
	...
	/** Sync, take the Workspace out of OPFS, and serve it at a root and at a subdirectory. */
	...
	test('references every asset relatively, asserted on the bytes that were written', async ({
		...
		// `grep -rEo '(src|href)="/[^"]*"'` over the sent Workspace, and `-r` is the point.
	...
	test('states the Base Map’s size before syncing, and adds those files', async ({ page }) => {
		...
		// The display assets' size is on screen before syncing spends it.
	...
	test('always syncs Base Map assets for a legacy site', async ({ page }) => {
		...
		// A legacy record may say the display assets were absent; the next Sync repairs the site.
	...
	test('announces progress from inside the modal, where the document is not inert', async ({
		...
		expect(
			await page.evaluate(() => {
				...
				// in the Sync, which is the whole reason the region is asked about rather than the
		...
		// rather than long enough to be lucky. Syncing fetches each one from the editor's deployment.
		...
		// a screen reader would read out again on the next Sync.
	...
	test('extends the hub page on a second Sync and leaves the first Project untouched', async ({
		...
		// "syncing wrote nothing of the user's" would stop covering the pyramid, which is most of it.
	...
	test('refreshes a version stamp that has gone stale, and says so before it is refreshed', async ({
		...
		// A Workspace whose site an older build of the editor wrote, which the next Sync brings up to date.
		await seed(page, {
...
// three budgets — is asserted in `packages/core/src/remote/send-to-remote.test.ts`, where the
...
// mode a Sync has is silent and plausible, and a test counting requests passes over all of them.
// The one exception is `blobPosts`, which measures what was *sent* — "the second Sync uploaded
...
test.describe('syncing to a Remote', () => {
	...
	async function start(
		...
		// connected, and pressed to Sync with nothing held. `signedIn` is what moves past it.
		await openWorkspace(
			page,
			{
				...projectFiles('amsterdam-1625', { name: 'Amsterdam 1625' }),
	...
	/**
	 * Reach the Sync dialog of a bound Workspace that holds a credential.
	 *
	 * ⚠ **The credential is seeded rather than acquired, and that is a statement about what these
	 * tests are for.** On a deployment with a GitHub App the Sync dialog offers no token field —
	 ...
	 */
	async function sendToRemote(page: Page, dialog: ReturnType<Page['getByRole']>) {
		await expect(dialog.getByTestId('sync-site-breakdown')).toBeVisible();
		await dialog.getByTestId('sync-send').click();
	...
	test('sends the Workspace to its Remote, .nojekyll and all', async ({ page }) => {
		const github = await start(page, {
			...
			// namespace, so a Sync must leave both exactly where they are (ADR-0033).
			hosts: {
				repositories: [
					{
						owner: OWNER,
		...
		// The tile's bytes, not merely its path: a Sync that sent every file empty would pass a
		...
		// And nothing of the scholar's own was sent over. This is the assertion that fails when the
		// owned namespace is one segment too wide, and it fails silently in production: a `CNAME` gone
		// moves a cited address back to a `github.io` URL, and the next Sync does it again.
		...
		// ⚠ **What it holds is the *source* this Sync wrote, and not merely the paths it uploaded** —
		...
		// `ballastella-site.json`, the Base Map's fonts and sprites — is site-owned output: it is
	...
	test('says nothing needed changing on a second Sync, and sends no blob', async ({ page }) => {
		const github = await start(page);
		await sendToRemote(page, await signedIn(page));
		...
		// `SyncDialog`'s header rules out, and this is the same rule applied to the other way of
	 ...
	 *
	 * Three numbers, because a Sync can be slow for three different reasons and a scholar cannot
	...
	test('announces files done, files total and the requests left, and keeps focus', async ({
		...
		// The total the line is counting towards, kept so the finished Sync can be held to it.
		...
		// the Sync (WCAG 2.4.3). It is the control this Sync was started from — the door closed
	...
	test('refuses a truncated tree, quoting the file count, and sends nothing', async ({ page }) => {
		...
		// Sync that did not look would upload everything again and then commit a tree missing most
	...
	test('warns before starting when it needs more requests than remain, naming the reset', async ({
		...
		// ⚠ **A budget the Workspace fits into and the Sync does not, which is the whole point of
		...
		// the folder as it stands says this Sync fits, and the scholar meets the 403 three hundred
		...
		// — `send-to-remote.test.ts`, "warns when the site would pass the static-hosting limit" —
		// because provoking it from a browser means a Workspace of over a gigabyte, and a Sync hashes
	 ...
	 * before a byte moves, and `sendToRemote` plans again afterwards against the Workspace the local
	 * Sync has just refreshed. So both halves are tied down here — the stated total against the tree
	...
	test('stops legibly when the budget runs out part way, leaving the site as it was', async ({
		...
		// Nothing is visible on a Remote until the ref moves, so an interrupted Sync has to leave the
	...
	test('lands on connecting, not on syncing, for a Workspace bound to nothing', async ({
		...
		// Signed out as well as unbound, which is the state a Workspace with nowhere to Sync is
		...
		// ⚠ **And nothing is asked of GitHub to find that out.** A Workspace with nowhere to Sync
	...
	/**
	 * A bound Workspace without a credential is offered the sign-in, and no way to send.
	 *
	 ...
	 * empty or disabled.
	 *
	 * ⚠ **The plan is still read, anonymously, which is what gives a signed-out author a *To get*
	 * column at all** (ADR-0044) — so the claim about GitHub is *which* requests were made rather
	 * than that none was. Every one is an anonymous read of this repository, and nothing went near
	 * the sign-in or the broker: a credential is what may not be asked for or sent here, not a
	 * request. Read after the plan has settled — while `sync-reading` stands, the requests are still
	 * arriving, and a count taken before it clears is a race rather than a claim.
	 */
	test('requires sign-in before sending from a bound Workspace', async ({ page }) => {
		...
		// The one press a signed-out author does get, which is the whole reason the plan was read.
		await expect(dialog.getByTestId('sync-get')).toBeVisible();
		await expect(dialog.getByTestId('sync-reading')).toHaveCount(0);
		expect(
	 ...
	 * stays asserted at Seam 1, in `send-to-remote.test.ts`'s permission check.
	 *
	 ...
	 *
	 * So closing is not what makes it stale — switching Workspace is. "Sent to ada/atlas"
	 ...
	 * is the first credentialed request a Sync makes and it sends nothing — so a scholar meets it
	...
	test('says the sign-in has expired, offers the way back in, and forgets the credential', async ({
		...
		// Not "GitHub refused this send", which sends a scholar off to check a repository that is
