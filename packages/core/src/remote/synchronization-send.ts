...
//
// `send-to-remote.ts` sends bytes and answers what it sent. This is where that answer becomes
...
//      commit-consistent Remote inventory. `planRemoteSend` posts nothing, so a read-only
...
// after the Remote write succeeds, the Send succeeded and the status is now Cannot tell: never
// the Send reported as failed, and never stale evidence retained. So a refused
// write is answered as {@link WorkspaceSent.baselineKept} `false` rather than thrown, and
// `writeBaseline` has already discarded the record that describes the state before — a stale map
// the reader could not tell from a current one is every path this send legitimately changed
...
/**
 * Whatever can narrow the local-change index to what has not been sent.
 *
...
export interface SendWorkspaceOptions {
	 ...
	 * one — so it sends with no evidence either way and {@link WorkspaceSent.baselineKept}
	 * stays `true`.
	 */
	 ...
	 * another machine sent in the window, deleted without anybody having seen its name.
	 *
	...
	/** What the local Published Site write will add, for the forecast's three budgets. */
	readonly pending?: readonly PendingLocalFile[];
}

/** What a send did, and what this installation now believes about it. */
export interface WorkspaceSent {
	 ...
	 *
	 * `false` is a successful send this browser could not record, which is `Cannot tell` — and
	 * it is never a failed send. The caller has to say so out loud: met unexplained on the next
	 * send, that `Cannot tell` is a refusal nobody can account for.
	 */
 ...
 *
 * @throws RemoteSendRefusedError for a read-only account, a repository nobody can see, a
