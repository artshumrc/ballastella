...
/**
 * The file a Layer draws, by path within its Project, or `''` when this build cannot say.
 *
 * The one place that maps a Layer's kind to the file that has to be deleted with it and restored with
 * it, so the delete and the undo cannot disagree about which file that is. A {@link
 * import('../project/layer.js').ForeignLayer} answers `''`: its reference is a field in
 * `unknownFields` whose name this build has never heard of, and guessing would either orphan a file or
 * delete one we were never asked to touch.
 */
export function layerFileRef(layer: Layer): string {
	switch (layer.kind) {
		case 'map':
			return layer.alignmentRef;
		case 'annotation':
