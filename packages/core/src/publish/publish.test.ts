...
describe('publishing', () => {
	...
	it('writes the repository into the record and into no document of its own', async () => {
		...
		expect((await readPublishedSite(store))?.repository).toEqual({
			owner: 'ada',
			repository: 'atlas',
			branch: 'main'
		});
...
describe('taking the Published Site back out of a Workspace', () => {
	...
	it('removes the whole recorded viewer file set, `_app/` included', async () => {
		await withSite();
	...
	// The other half of the same claim, said as bytes: the scholar's files are the ones a withdrawal
	// must be incapable of reaching.
	it('leaves every source file byte-identical', async () => {
		await withSite();
		const before = new Map<string, string>();
		for (const path of await store.list('')) {
			if (!path.startsWith('_app/') && !path.endsWith('.json') && path !== '.nojekyll') continue;
			before.set(path, decode(await store.read(path)));
		}

		await withdrawShareLinks(store);

		expect(decode(await store.read('amsterdam-1625/project.json'))).toBe(
			before.get('amsterdam-1625/project.json')
		);
		expect(decode(await store.read('alignments/x.json'))).toBe('{"type":"Annotation","id":"x"}');
		expect(decode(await store.read('images/x/info.json'))).toBe('{"id":"https://unset.invalid/x"}');
	});

	// Idempotent, because the presses that reach it are: a Workspace that never had a site is the
	// state a withdrawal wants, not an error.
