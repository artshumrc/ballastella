...
// Connecting, Share Links and the credential, reachable through the same barrel: the editor is the
// only caller of all three, and it imports nothing from `packages/core` except this file. The
// credential store is here rather than in the app so that the one rule ADR-0033 states about it —
// sealed while a Review Workspace is open — has a test seam the app has not got.
it('exposes connecting to a Remote, Share Links and the credential store', () => {
	expect(Object.keys(core)).toEqual(
		expect.arrayContaining([
			'awaitRemotePages',
