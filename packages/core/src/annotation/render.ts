...
/** How wide a `stroke-width` of 1 draws a dash of 1, so a dotted line reads as dots at any width. */
const DASH_UNIT = 1;
 ...
 * `stroke-dasharray` — expresses it in the same units as the width itself. So a stored `[8, 4]` on a
 * 2px line means 8px on and 4px off, which MapLibre needs as `[4, 2]`. Dividing by the width is that
 * conversion, and it is why a dotted line stays dotted rather than becoming a dashed one when the
 * user makes it thicker.
 */
export function mapLibreDashArray(
	dash: readonly [number, number],
	strokeWidth: number
): [number, number] {
	const width = strokeWidth > 0 ? strokeWidth : DASH_UNIT;
