...
The editor says the same thing the published viewer now says when a Historical Map's tiles stop arriving — on the Project screen, and on the alignment screen, where it matters most.
...
- `apps/editor/src/lib/alignment/AlignmentWorkspace.svelte` and `apps/editor/src/lib/image-pane/HistoricalMapPane.svelte` — the alignment screen and the pane that reads a Historical Map. The pane already carries an offline notice naming the host, and already has a load effect with an `untrack`ed connection read whose reason is written down. **Read that reasoning before touching the effect** — losing the connection must not tear down a pane a scholar is mid-alignment on.
...
- [ ] With a referenced Historical Map's tiles refused mid-alignment, the alignment screen shows the sentence, and the Control Points already placed are still on screen and still on disk.
...
- Ticket 04 — a Reader is told when a Historical Map's tiles stop arriving.
