...
10. **A service worker can supply the COOP/COEP headers open question 3 is blocked on — measured, deliberately not implemented.** This is the most consequential finding in the epic's last wave, because it is a possible route out of the `wasm-vips` wall, and it is a decision rather than a task.

    Ticket 18 measured it against a host sending no COOP and no COEP, with a worker re-serving navigations carrying `COOP: same-origin` + `COEP: credentialless`:

    | Document | `crossOriginIsolated` | `new SharedArrayBuffer(8)` |
    | --- | --- | --- |
    | first load (uncontrolled) | `false` | throws |
    | once the worker controls it | **`true`** | **succeeds** |

    With ticket 05's own measurement — COOP/COEP present → `Vips()` initialises, libvips 8.18.3 — the route is real, and the plumbing is two lines in a function that already answers navigations from cache.

    **Two reasons a human may still say no.** `require-corp` breaks no-cors cross-origin loads, and OpenSeadragon's default loader is `<img src>`, so the unwarped view of a library that sends no CORP would stop working; `credentialless` fixes that and **Safari does not implement it**, while SPEC story 4 makes Safari first-class. And the pattern needs the app to reload itself on a first visit, which is precisely the behaviour story 9 argues against. **Ticket 05's option 1 — vendor a single-threaded `wasm-vips` build — needs no headers and moots both objections.** Decide between them rather than treating this as a free win.
...
- **An exception in any Svelte teardown abandons the rest of the destroy flush *and* the next page's mount.** This is the root cause of a defect that made a client-side navigation to `/layers` render nothing at all, and it will recur. Svelte destroys effects in **creation order**, and `onMount` is registered above every effect in `BaseMapPane.svelte` — so `Map#remove()` runs first, and maplibre-gl 5.24's `remove()` does `setStyle(null)` → `delete this.style`, while `Map#getLayer(id)` is `return this.style.getLayer(id)`. The teardowns call `getLayer` precisely *because* a theme change's `setStyle` may already have removed their layer, so the guard written for one hazard became the throw for another. The throw then abandoned the synchronous flush that contained the destination page's `onMount`.
  **It was invisible in both directions.** Navigating the other way threw too, but the Project page's markup is derived state, so it rendered and only its effects were skipped; the existing "links back to the Project" test passed throughout. Fixed at the two throwing sites rather than with a blanket guard — but **ticket 17 adds more navigation between map-bearing panes, and a `pageerror` assertion on those navigations is cheap insurance.**
- **`$service-worker`'s lists are *file* paths; requests carry *URL* paths.** Ticket 18 found the glyph directories named `Noto Sans Regular` were therefore a cache miss on every request — **and MapLibre's answer to a glyph it cannot fetch is a `console.warn` plus a local system font**, invisible to every assertion about the map. Any future cache list has the same trap.
- **`registration.active.state === 'activated'` is not "will control the next navigation".** Reloading in that window leaves a page *permanently* uncontrolled, and it reads as "the worker did not install". `navigator.serviceWorker.ready` is the correct wait.
- **A claimed dead assertion that was not dead — verify before fixing.** Ticket 16 reported `e2e/editor-image-pane.e2e.ts:86` as a missing `await`, so "that assertion cannot fail the test". It is an arrow function *returning* the `expect(...)` promise, and **all six call sites await it**. Falsifying the expected value fails 5 tests, so the assertion is live. Recorded because the report was plausible, the shape is one this epic really does keep hitting, and the mutation check took two minutes — which is the right response to any "this test asserts nothing" claim, including a true one.
- **A whole class of navigation bug was invisible because every test used one entry route or the other, never both.** Both Layers-pane defects were found only because ticket 15 happened to need the link route for one control and the direct-load route for another. `openLayers` now takes the way in as a parameter and both routes are exercised. Treat "reached by link" and "reached by load" as distinct states worth asserting wherever a pane builds something on mount.
