...
Tar, not zip, because `fflate` counts entries in sixteen bits and emits no zip64 — measured, 70,000 entries indexed as 4,464 and read back with no error — and as a whole-Workspace backup, exceeding that is the normal case. Tar also streams both ways, which is what makes restoring on an iPad possible at all. `modern-tar` is the intended dependency; a WASM one is rejected on this repo's own `wasm-vips` history. **USTAR `prefix`/PAX handling is load-bearing**: `<project-dir-up-to-64>/annotations/<uuid>.geojson` is about 121 characters, past tar's 100-character name field.
...
- **The COOP/COEP service worker** that would unblock `wasm-vips` for images above ~268 MP. Measured and confirmed to work; deliberately not built. It is a site-wide restriction on how the page may load other origins' files — a real risk to referenced IIIF tiles and the Base Map archive — bought for a rare case that already fails legibly. **This is fenced explicitly because the epic touches the service worker for the tile cache, and the temptation will be adjacent.**
...
**What it does not close.** The canonical instance URL; whether Playwright should run on Firefox and WebKit; `wasm-vips` on a static host; upstreaming the `@allmaps/render` patch; the two — now three — licence texts that do not ship. ADR-0025's build-fence pattern is a useful precedent for the canonical-URL question, which is the same shape: deployment configuration with a guard.
