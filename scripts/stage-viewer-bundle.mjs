#!/usr/bin/env node
// Put the built read-only viewer inside the editor, so that Share Links has something to write.
//
// ADR-0045: a Published Site is an `index.html` and the viewer's files inside the user's Workspace.
...
// at all, which is Share Links fetching 404s.
//
// The index records, per file, where the editor serves it from and where the site write puts it. Those
...
// every deployment of the editor to no purpose. Their `source` and their site `path` are the
...
/** The Base Map's deployment assets, whose site path is the same as their served path. */
...
// the index does not name and the site write therefore never overwrites.
...
// Beside the staged files rather than inside the list, so the index is not itself written into a site.
