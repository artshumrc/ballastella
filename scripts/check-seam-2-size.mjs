 ...
 * | 2026-08-31 | 664 | Deleting the Workspace settings and Remote repository dialogs (ADR-0042) retires twelve Seam 2 tests and adds two, so the ceiling comes **down** twelve. What comes out: two that drove the settings dialog as a dialog — that it was a real `<dialog>` with focus restoration, and that the folder choice was reachable from it *and nowhere else*, which is the opposite of the arrangement that replaced it; one about the single pre-plural folder slot being let go by *Use browser storage instead*, a control that no longer exists because switching to a browser Workspace is pressing its row; and nine that could only be driven through a token field or an address field, both of which are absent on a deployment with an App — the pasted credential's four (a paste of the wrong shape, a paste GitHub refuses, where a paste is kept, and a paste supplied again for a bound Workspace), the escape hatch's own test and the fork-shaped paste beside it, a typed whole-address bind and a typed repository that does not exist, and binding-with-a-notice on a repository that cannot be published to. Every one of those claims is asserted at Seam 1 (`credential-store.test.ts`, `bind-remote.test.ts`, `github-sign-in.test.ts`) or at Seam 1c against the reactive fake (`connect-to-github.dom.test.ts`, `repository-choice.dom.test.ts`), except the escape hatch itself: **that surface was re-established on the door, with a Seam 2 test of the real `isGitHubAppConfigured` gate behind it**, which is the raise the row below spends. What goes in: *moves this Workspace into a folder, preserving every file and keeping the original*, which is the only route existing work has onto disk and needs a real granted `FileSystemDirectoryHandle` and a real OPFS Workspace to compare byte for byte; and the refusal beside it, a folder that already holds files, which is what keeps a move from being a merge. Workspace Home's re-homed sections moved *down* a seam rather than out: `keeping-your-work.dom.test.ts` is twenty-two tests at Seam 1c where sixteen browser boots used to be. |
 * | 2026-08-31 | 666 | The raise the row above said the escape hatch would need: it is back on the door itself, and the one Seam 2 test that goes with it is *the way in behind the disclosure binds with no request to the broker*. Three things meet there that no lower seam holds together: that the disclosure is gated on the **real** `isGitHubAppConfigured(GITHUB_APP)` rather than on a fake's field, that the token field is absent from the document until it is pressed, and that the bind behind it reaches GitHub with every request to the broker's routed origin failing — "which host was reached" being a question a fake store at Seam 1c cannot answer. Everything else about the disclosure — that it is closed, that it says so, what it binds with, and that it is not offered where there is no App — is at Seam 1c in `connect-to-github.dom.test.ts`. |
 * | 2026-08-31 | 674 | Eight tests for the keyboard-only pass across the whole of the interface the bar, the door and the roster present, in `editor-keyboard.e2e.ts`. Every accessibility claim that can be made of one surface is already at Seam 1c and stays there — focus restoration in `modal-dialog.dom.test.ts`, a busy control keeping its place in `keeping-your-work.dom.test.ts`, the badge's single status region and each disclosure beside the disclosure. What is here is the one property none of them can see: that `Tab` reaches the bar's badge disclosure, the door, the roster and every control ADR-0042 re-homed onto Workspace Home, in that order, and that `Enter` operates each and comes back. `Tab` is the subject, and happy-dom implements no sequential focus navigation at all — a `Tab` there is a keystroke nobody answers, so the whole pass would be vacuously green. The focus trap `showModal()` brings, the popover's own focus scope, and the order layout produces exist only in a real engine. |
