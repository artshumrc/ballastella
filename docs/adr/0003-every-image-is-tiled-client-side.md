...
> **The streaming clause is superseded by [ADR-0027](./0027-no-streaming-tiler-in-v1.md):** there is no second tiler. `wasm-vips` is removed and an image above the measured `createImageBitmap` decode ceiling is refused by name rather than routed. npm publishes only the threaded build, which needs COOP/COEP headers GitHub Pages cannot send, so the "use the single-threaded build" instruction below asks for an artefact that does not exist and the path never ran. Everything else here still holds — one contract, `getTileImageRequest` serving both sides, square tiles, ingest as a job — and the `sharp` CLI in prose is now the whole of the escape hatch rather than the last resort behind libvips.
