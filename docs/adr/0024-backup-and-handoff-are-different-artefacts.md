...
- **A backup excludes the published viewer files.** `index.html`, `_app/`, and `ballastella-site.json` are build output that `isViewerFile` already enumerates and `exportProjectZip` already skips for its data-only flavour. Including them would bloat every backup and restore a viewer bundle possibly older than the app, which goes stale against its data. A restored Workspace therefore needs Share Links turned on again to be a site ([ADR-0045](./0045-a-repository-holds-the-work-and-a-site-is-asked-for.md)), and the restore must say so rather than letting the user find a stale site.
