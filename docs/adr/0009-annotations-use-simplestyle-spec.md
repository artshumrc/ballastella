...
## A Circle is a polygon with a semantic foreign member

GeoJSON has no Circle geometry. A Circle is therefore written as a 64-segment Polygon so ordinary GIS software can draw, style, and inspect it, plus the Feature foreign member `ballastella:circle: { center: [lng, lat], radiusMeters }`. The foreign member follows RFC 7946 §6.1 rather than entering `properties`: scholarly properties remain simplestyle content, while the center and radius describe how the geometry is edited.

The center and radius are the editable truth. Moving either of the Circle's two handles regenerates every polygon position geodesically, so resizing cannot turn it into an arbitrary polygon. A consumer that ignores the foreign member still receives the portable Polygon; a build that understands it presents a Circle and preserves the invariant.

Terra Draw's Circle mode was reconsidered and rejected. It also stores an approximated Polygon, does not persist the center, can resize that Polygon non-uniformly, and can leave its radius property stale after editing. It therefore does not supply the semantic invariant, one-write gesture, or keyboard-operable handles that this application needs, while overturning ADR-0005's settled dependency decision.
