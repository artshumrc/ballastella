...
The wrapper is MIT, but the artefact it ships is compiled **libvips, which `wasm-vips` states as LGPLv3** — reached via the "any later version" clause of LGPLv2.1, so LGPLv3's terms are the ones that bind, not LGPL-2.1's. This decision originally said LGPL-2.1-or-later, and that was checked against the installed package and corrected once the dependency was real.
