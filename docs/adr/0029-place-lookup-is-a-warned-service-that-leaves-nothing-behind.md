...
**An "unsettled" flag** survived that objection and was still declined. It would record a claim by the *scholar* — "I have not checked this yet" — cleared by exactly the action that falsifies it, so it cannot go stale. It answers a real question: forty Pins across three sessions, which ones did I verify? And it had a natural home, since the Publish dialog already "warns you about anything that would make the site disappoint a reader."
