...
> **Amended by [ADR-0034](./0034-a-shared-ui-package-for-the-components-both-apps-render.md):** the seam this ADR said to wait for has proved itself. `packages/ui` holds the Svelte components and the stylesheet both apps render, publishing source with `svelte` as a peer dependency and no build step, exactly as `core` does. The rejection of `packages/{domain,store,iiif,map,ui}` below stands as written — that was a decomposition invented before the code existed, and this is a boundary drawn around code that already existed twice. Nothing else here changes: two builds, and the fence below now walks the new package too.
