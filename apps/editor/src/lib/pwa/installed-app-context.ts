...
//
// So the `if` was deletable with the whole suite green — exactly the shape of untested safety a
// review has to catch by eye. A function of one argument fixes that for the cost of one file and
// this note.
//
 ...
 * is a second pair of `online`/`offline` listeners, banned outright, and two answers to "is there a
 * connection" is how a pane and a Layer card come to disagree in front of a scholar.
 *
 * It is worth having because `MapImagePane` reads `.online` in its load effect, on a component that
 * is reusable by design: without the guard a missing context surfaces as `Cannot read properties of
 * undefined (reading 'online')` thrown from inside an effect, naming nothing.
 */
