...
<script lang="ts">
	...
	let {
		...
		entryId: string;
		/**
		 * How the geography is drawn: streets, satellite, relief, high contrast. The Project's, owned and
	...
	const paintKey = (
		...
		`${id}@${currentTheme}@${cachedTo ?? 'network'}@${drawn.streets}${drawn.relief}${drawn.highContrast}${drawn.imagery}@${drawnBorders}@${drawnStyle.color ?? 'auto'}@${drawnStyle.lineStyle ?? 'auto'}@${drawnStyle.width ?? 'auto'}`;
