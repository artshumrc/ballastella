...
import { historicalMapSourceOf, warpedAddressOf } from './map-source.svelte.js';
...
describe('where the Historical Map being aligned is served from', () => {
	it('does not re-run its readers when an unrelated Workspace read has answered', () => {
		// A stand-in for `EditorSession.remoteOrigins` — the `$state` behind `historicalMapSource`,
		...
		const lookUp = (imageId: string): HistoricalMapSource => {
		...
		let runs = 0;
		let seen: HistoricalMapSource | undefined;
		inARoot(() => {
			const source = historicalMapSourceOf(lookUp, () => IMAGE_ID);
			$effect(() => {
				seen = source.current;
				runs += 1;
			});
			...
			// this is `refreshAddableHistoricalMaps` in another Project's dialog, or a Workspace refresh,
...
describe('the address the warped renderer is built from', () => {
	it('does not rebuild the warped layer when the source object is merely re-made', () => {
		let source = $state.raw<HistoricalMapSource | null>({
